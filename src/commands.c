...
void parseCommandLine(char *command)
{
	...
	if (cmdonly)
	{
		...
		else if (!strcmp(cmdonly, "set"))
		{
			if (argument)
			{
				//check if var_name is given!!!!
				//check if name and value is at most 16 chars
				char *var_name = argument, *var_val = argument;
				...
				*(var_val - 1) = '\0';
				...
				if (*var_val)
				{
					...
					setStatusMessage(0, "set %s to %s", var_name, var_val);
					set_var(var_name, var_val);
				}
				else
				{
					setStatusMessage(0, "set %s to default value?", var_name);
				}
			}
			else
			{
				setStatusMessage(0, "all: %s", get_all_vars());
			}
		}
